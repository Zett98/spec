...
let gas_constant_cost gas =
  let constant = 100 in
  if !gas >= constant then gas := !gas - 100 else  Exhaustion.error no_region "Out_of_gas"

let rec eval gas (c : config) : value stack =
  match c.code with
  | vs, [] -> vs
  | vs, { it = Trapping msg; at } :: _ -> Trap.error at msg
  | vs, es -> 
    gas_constant_cost gas;
    eval gas (step c)

(* Functions & Constants *)

let invoke (gas : int ref) (func : func_inst) (vs : value list) : value list =
...
let create_global gas (inst : module_inst) (glob : global) : global_inst =
  let { gtype; ginit } = glob.it in
  let v = eval_const gas inst ginit in
...
let create_elem gas (inst : module_inst) (seg : elem_segment) : elem_inst =
  let { etype; einit; _ } = seg.it in
  ref (List.map (fun c -> as_ref (eval_const gas inst c)) einit)
...
let init gas (m : module_) (exts : extern list) : module_inst =
  let {
    imports;
    tables;
    memories;
  ...
  let inst2 =
    {
      ...
      globals = inst1.globals @ List.map (create_global gas inst1) globals;
    }
  in
  let inst =
    {
      ...
      elems = List.map (create_elem gas inst2) elems;
  ...
  ignore
    (eval gas (config inst [] (List.map plain (es_elem @ es_data @ es_start))));
  inst
